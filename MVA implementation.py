...
"""
Title: "Sample implementation of Minimum Variance Algorithm (MVA)"
Created: 2020-05-19
...
"""
-The following is an implementation of the MVA - without using a function.

-The program is just an example of how the MVA can be implemented in the
financial field.

-The following implementation uses 'iexfinance' package and it requires a valid
token in order to function properly.

-I will leave my test token (sandbox) in the following program.
Please do NOT abuse of this token, as it is provided for example only. You have
to replace it with your own token.

-The test token uses randomized data.

-The following program is provided 'as is' and it will not work out-of-the-box,
...
#set token - iex cloud
os.environ['IEX_TOKEN'] = 'Tpk_8698c6e7b1a34ee98b6de1d560880f70' #test data
...
'''
#Example of iexfinance usage

batch = Stock(['TSLA', 'AAPL', 'F'])
print(batch.get_quote())
#print(batch.get_quote().loc['extendedPrice']
'''
...
print('\nHello Investor.')
print('\nI will show you an sample output of the Minimum Variance Algorithm.\n')
...
securities_list = [ dia, qqq, spy ]
securities_list.sort()   

#Prepare toplist - which will be 'passed' to the MVA as a tuple
toplist = tuple(securities_list)
